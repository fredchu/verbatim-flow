...
- **多引擎支持** — Apple Speech、本地 Whisper、OpenAI 云端、Qwen3 ASR、MLX Whisper（本地，Apple Silicon）
...
| **引擎可切换** | Apple Speech / Whisper (tiny–large-v3) / OpenAI Cloud (gpt-4o-mini-transcribe, whisper-1) / Qwen3 ASR (0.6B / 1.7B，Apple Silicon 本地) / MLX Whisper (Large V3，Apple Silicon 本地) |
...
- **多引擎运行时切换** — Apple Speech / Whisper / OpenAI 云端 / Qwen3 ASR / MLX Whisper，菜单内一键切换
...
- **中英混合增强** — 专门的混合语言后处理优化
- **语言选择** — System Default / zh-Hans / zh-Hant / en-US
...
- Xcode 16+
- 麦克风和辅助功能权限
- Apple Silicon（M1/M2/M3/M4）或 Intel Mac（支持 Universal Binary）
...
│   ├── python/                  # Python ASR 脚本
│   │   ├── scripts/             # CLI 入口（transcribe_qwen.py, transcribe_mlx_whisper.py）
│   │   └── verbatim_flow/       # Qwen3 ASR 及 MLX Whisper 转写模块
│   ├── Package.swift
│   └── dist/                    # 构建产物（.app, .dmg）
├── packages/                    # 共享包占位
├── scripts/
...
- [x] 原生 Swift 路径集成 Whisper 引擎
- [x] 中英混合识别优化
- [x] Qwen3 ASR 本地引擎（0.6B / 1.7B，Apple Silicon）
- [x] MLX Whisper 本地引擎（Large V3，Apple Silicon）
- [x] 繁体中文（zh-Hant）语言选项
...
- [Qwen3-ASR](https://github.com/QwenLM/Qwen3-ASR) — 阿里 Qwen 团队的多语言 ASR 模型
- [mlx-audio](https://github.com/Blaizzy/mlx-audio) — Apple Silicon 优化的音频 ML 框架
- [mlx-whisper](https://github.com/ml-explore/mlx-examples/tree/main/whisper) — Apple Silicon 上通过 MLX 运行 Whisper 推理
